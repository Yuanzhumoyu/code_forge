...
| ABI | **>128 位向量 by-ref 传参 + sret 返回已支持**（2026-08-31 起 ed43103+）；≤128 位按值有 XMM 全宽缺口 | Windows x64 无 YMM 参数寄存器——>8B 非标量按引用传指针（占 GPR 槽）、返回 >64 位走首参 RCX 隐藏 sret（forge by-ref/sret 即原生形态，jit 7 测试全绿：v256/v512 byref param、wide_vector_call byref/sret、mixed/sret_with_byref）。**残余**：≤128 位向量（V64/V128）按值参数走 MOVSD/MOVSS（8/4B 静默截断，forge-rustc B3 门控保护中）；宽向量第 5+ GPR 槽显式 Unsupported；V512 测试仅断言 lane0。见 forge-rustc WORKAROUNDS.md WA-37 |
