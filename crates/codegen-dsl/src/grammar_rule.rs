...
//!
//! Parses EBNF-like rule strings into lang_frontend::Expr nodes.
//! Supports: sequences, alternation (|), repetition (* + ?), grouping, tokens, literals.

use lang_frontend::Expr;
...
impl RuleExprParser {
    ...
    pub fn parse_alt(&mut self) -> Result<Expr, String> {
        ...
        if alts.len() == 1 { Ok(alts.pop().unwrap()) }
        else { Ok(Expr::alt(alts)) }
    }
    ...
    fn parse_seq(&mut self) -> Result<Expr, String> {
        ...
        if items.is_empty() { Ok(Expr::Epsilon) }
        else if items.len() == 1 { Ok(items.pop().unwrap()) }
        else { Ok(Expr::seq(items)) }
    }
    ...
    fn parse_atom(&mut self) -> Result<Expr, String> {
        ...
        while self.pos < self.tokens.len() {
            match self.peek() {
                Some("*") => { self.advance(); expr = Expr::ZeroOrMore(Box::new(expr)); }
                Some("+") => { self.advance(); expr = Expr::OneOrMore(Box::new(expr)); }
                Some("?") => { self.advance(); expr = Expr::Opt(Box::new(expr)); }
                _ => break,
            }
        }
