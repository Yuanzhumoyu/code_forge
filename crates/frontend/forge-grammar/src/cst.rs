...
/// Transparent structural wrapper kinds that are skipped during flattening.
/// These are EBNF combinator labels (Seq, Alt, ZeroOrMore, OneOrMore, Opt)
/// that don't carry semantic meaning in the CST.
const TRANSPARENT_WRAPPERS: &[&str] = &["seq", "alt", "rep", "rep1", "opt"];
...
impl CstNode {
    ...
    // ============================================================
    // Flatten methods — skip transparent EBNF wrapper nodes
    // ============================================================

    /// Check if this node is a transparent structural wrapper.
    fn is_transparent_wrapper(&self) -> bool {
        TRANSPARENT_WRAPPERS.contains(&self.kind.as_str())
    }

    /// Flatten this node: if it's a transparent wrapper with exactly one child,
    /// return that child (recursively flattened). Otherwise return self.
    ///
    /// This skips EBNF combinator wrappers like "seq", "alt", "rep", "opt"
    /// that don't carry semantic meaning, exposing the underlying rule/token node.
    pub fn flatten(&self) -> &CstNode {
        let mut current = self;
        while current.is_transparent_wrapper() && current.children.len() == 1 {
            current = &current.children[0];
        }
        current
    }

    /// Find a child by kind, skipping transparent wrappers recursively.
    /// Returns the first child where `child.flatten().kind == kind`.
    pub fn flatten_child(&self, kind: &str) -> Option<&CstNode> {
        for child in &self.children {
            let flat = child.flatten();
            if flat.kind == kind {
                return Some(flat);
            }
            // Search recursively inside transparent wrapper nodes (e.g. "rep", "opt")
            if child.is_transparent_wrapper()
                && let Some(found) = child.flatten_child(kind)
            {
                return Some(found);
            }
        }
    ...
    /// Find all children of a given kind, skipping transparent wrappers recursively.
    pub fn flatten_children_by(&self, kind: &str) -> Vec<&CstNode> {
        let mut results = Vec::new();
        for child in &self.children {
            let flat = child.flatten();
            if flat.kind == kind {
                results.push(flat);
            } else if child.is_transparent_wrapper() {
                // Search recursively inside wrapper nodes (e.g. "rep" with multiple children)
                results.extend(child.flatten_children_by(kind));
            }
        }
        results
    }
