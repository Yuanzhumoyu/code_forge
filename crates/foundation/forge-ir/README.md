...
| `opcode` / `immediate` / `inst_flags` / `mem_flags` | 指令操作码与附件 |
...
- **指令清单**：`Opcode::ALL` 是全部变体的单一事实源（覆盖率矩阵、名字查找都用它）；
  `Opcode::name()`/`from_name()` 用于 ISA TOML 的 `op = "..."` 契约。
