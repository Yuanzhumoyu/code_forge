...
//! 优先检查此处。适配版本：**rustc 1.99.0-nightly (1a98b1e13 2026-08-07)**。
//!
//! 升级流程：
...
//! # 漂移点清单（旧 API → 新 API）
//!
//! | 旧 API（≤2026-03） | 新 API（2026-08-07） | 位置 |
//! |---|---|---|
//! | `rustc_middle::mir::mono::MonoItem` | `rustc_middle::mono::MonoItem`（`mono` 提升为顶层模块） | lib.rs import |
...
//! | `LangItem::DropInPlace` | `LangItem::DropGlue` | call lowering |
