...
| WA-20 | 主库 regalloc（同块 def/use 重叠）+ forge-rustc Range 迭代 | `nested_loop_break_outer` 挂起根因（2026-08 定性）：spec_next 的 `_5 = copy ((*_1).0)` 中 **load(start) 的结果与 store 目标 lea 分配到同一寄存器 r15**——lea 覆盖 start 后 store 存了地址值 → _5 槽恒 0 → `Some(copy _5)` 解包 val 恒 0 / 主循环死循环（sentinel ExitProcess 0xDEAD 实证；反汇编 140001543 load→140001546 lea r15→14000154a store [r15] 实证寄存器重叠）——**与 WA-17（Select 的 mov rd,rs3 若 rd=cond 则 test 读被覆盖值）同源** | forge-rustc 侧：nested_loop_break_outer known_failure（判别层已修：ScalarPair tag 位宽 + switchInt 枚举判别；剩本 regalloc 问题）；主库 JIT 回归探针 test_jit_store_load_value_under_pressure（当前简单形态通过，未触发重叠） | 主库：regalloc 同块 def/use 重叠检查（def 的物理寄存器不得覆盖同块内仍在用的 use）；修后转正 nested_loop_break_outer |
