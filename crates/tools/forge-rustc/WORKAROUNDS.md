...
| WA-18 | 主库 AtomicRmw（isa TOML `xadd {g1}, [{0}]`）+ regalloc | 原子 RMW（fetch_add 等）运行期 SEGV：`xaddq %r10, (%r11)` 且 r11 从**未写槽**（-0x118）读——ptr 被 regalloc spill 但 store 缺失（di_atom 反汇编实证；与 WA-17 同源：mem 操作数寄存器在高压场景生命周期丢失） | forge-rustc 侧：原子 RMW intrinsic 编译期拒绝门控（失败即报错，atomic_load/store 保留）——`atomic_rmw_gated` e2e 用例守护 | 主库：AtomicRmw 的 mem 操作数 regalloc（ptr 值的 spill 写回缺失）；根治后移除门控转硬断言 |
