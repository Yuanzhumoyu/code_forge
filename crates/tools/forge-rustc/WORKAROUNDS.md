...
| WA-20 | 主库/forge-rustc Range 迭代链（spec_next→next→mainCRTStartup） | `nested_loop_break_outer` 挂起 + 多次 next 值错（2026-08 定性进展）：哨兵实证 **0..1 的第二次 next 返回 Some(0)（应 None）**——start 不递增；但 spec_next 反汇编**完全正确**（lt 返回 0/1、写回 movl→[Range 指针] 存在、next 包装 self 传递链正确、mainCRTStartup 的 &mut _1 传递正确）——**所有环节静态分析正确但运行期 start 不递增**——疑似主库 regalloc/emission 微妙 bug（需运行期单步工具，gdb PE 断点受限）；**单次 next（0..4 第一次）正确**（e2e range_next_once 转正）；for 循环形态死循环/值错（sentinel 0xDEAD 实证） | forge-rustc 侧：nested_loop_break_outer known_failure（判别层已修）；e2e range_next_once 转正探针；JIT 探针 test_jit_store_load_value_under_pressure（简单形态通过） | 待查：主库 regalloc/emission 在 Range 迭代形态的运行期值丢失（WinDbg/插桩）；修后转正 nested_loop_break_outer |
