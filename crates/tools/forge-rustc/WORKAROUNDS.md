...
| WA-20 | 主库/forge-rustc Range 迭代链（spec_next→next→mainCRTStartup） | `nested_loop_break_outer` 挂起（2026-08 定性进展）：sentinel 实证 **mainCRTStartup 循环 >1000 次（ExitProcess 0xDEAD）**；但 spec_next 反汇编（当前 dll）**完全正确**（load start→store _5 槽→lea 目标 r14/r15 分离→movabs 1→call forward_unchecked(start,1)）——**非确定性**：同源代码常量/变量 ExitProcess 参数编译产物行为分叉（sentinel 死循环 vs sentinel2 exit 0 且 s=0）——**疑似 Range 迭代链某处未初始化值**（next 包装返回/pack_sp 解包偏移），非单点可复现 | forge-rustc 侧：nested_loop_break_outer known_failure（判别层已修：ScalarPair tag 位宽 + switchInt 枚举判别）；JIT 探针 test_jit_store_load_value_under_pressure（主库简单形态通过） | 待查：next 包装（Range::next）的返回链 + mainCRTStartup 的 pack_sp/解包偏移（未初始化值定位）；修后转正 nested_loop_break_outer |
| WA-21 | 主库 store 值处理（高压形态）+ forge-rustc static 访问 | `static mut X` 写崩（2026-08 发现）：`_1 = const {alloc1: *mut i32}`（&X）的 store_place(_1) **缺失**——_1 槽（最终帧 -0x50）读垃圾（0x7ff6…）→ 写 [垃圾] SEGV；固定基址（/DYNAMICBASE:NO）仍崩（非 ASLR）；反汇编实证 mainCRTStartup 复杂形态（misaligned 检查 ×4 + &X ×2）下 GlobalAddr 的 store 未落槽——**与 WA-20 同源**（主库 regalloc/emission 对 store 值的处理在高压/多分支形态缺失）；简单形态（atomic fetch_add 的 &A store）正常；**vec_push（static mut HEAP allocator）偶发 SEGV 同因**（5 次单跑全过、全量偶发 FAIL） | forge-rustc 侧：e2e `static_mut_write` known_failure 回归探针（expect 12）；JIT 探针 test_jit_store_load_value_under_pressure（简单形态通过） | 主库：store 指令的值/目标在高压分配下的写回缺失（同 WA-20 根治方向）；修后转正 static_mut_write |
