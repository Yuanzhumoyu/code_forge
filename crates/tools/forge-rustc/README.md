...
- **M3 并行 CGU（Stage A，2026-09）**：`-C codegen-units=N` 只影响 rustc 的 CGU 划分——forge 在 `collect_instances`（按符号名稳定排序）后自管函数粒度 worker 池。任务体 = 连续函数块 + 任务私有 `FuncRefTable`（`@N/G{N}` 编号任务内分配、每函数编译完就地 resolve，零加锁）；`merge_task_table` 把任务登记条目（vtables/promoted 按 alloc_id 去重、line/var/enum 按全局函数序）归并回主表；主线程按原实例序串行 emission（单对象单模块，产物与 -C codegen-units 无关）。**当前宿主**（rustc 1.99+/WorkerLocal 查询引擎，[WA-38]）：`TyCtxt` 非 Send、查询无法跨自定义线程 → 默认串行；`FORGE_CODEGEN_THREADS>1` 显式启用时经跨线程查询能力探针，不支持则自动回退串行（行为不变）。
...
| **并行 CGU（`-C codegen-units=N`）** | **2026-09 M3 Stage A 已落地（默认串行 + env 显式并行，WA-38）**：rustc 对自定义后端只调 codegen_crate 一次、**无按 CGU 回调接口**（逐 CGU 并行是 LLVM 在自己 codegen_crate 内实现，需 ExtraBackendMethods）→ 自管 worker 池架构按方案落地：A1 排序实例的函数切连续块 + `std::thread::scope` 分块任务（每任务私有 FuncRefTable——编号 `@N/G{N}` 每函数编译完就地 resolve 不跨函数逃逸，零加锁）+ 任务登记条目 `merge_task_table` 归并（vtables/promoted 按 alloc_id 去重、line/var/enum 按全局函数序；slice_sym 命名纯化 `__slice_{alloc_id:?}`）+ 主线程按原实例序串行 emission（单对象单模块、产物与 -C codegen-units 无关，determinism cgu=1/2/4 交叉矩阵守护）。**宿主限制**：rustc 1.99/1.100（统一 WorkerLocal 查询引擎）`TyCtxt` 非 Send、查询只能在 rustc 自身查询池线程执行（WA-38）→ 默认 T=1；`FORGE_CODEGEN_THREADS>1` 显式启用并行，先经跨线程查询能力探针，不支持则自动回退串行（host 恢复后自动点亮） | Stage B（后续）：每 CGU 独立 ObjectWriter 多对象 + WorkProduct（dwarf 需 per-CGU CU 或 debuginfo 回退）；并行点亮需 rustc 提供 codegen 后端线程注册入口 |
