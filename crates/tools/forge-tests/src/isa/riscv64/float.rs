//! riscv64 浮点指令测试（test-float feature 门控）— compile-only。
//!
//! riscv64 无本机执行；此处验证代表性浮点指令组合能编译成功，
...
/// compile-only 断言（riscv64 TargetMachine，返回 f64）。
...
fn float_arith_chain() {
    compile_ok_f64("riscv64_float_arith_chain", |b| {
...
#[test]
fn float_sub() {
    compile_ok_f64("riscv64_float_sub", |b| {
...
fn float_unary_chain() {
    compile_ok_f64("riscv64_float_unary_chain", |b| {
...
#[test]
fn float_compare() {
    compile_ok_f64("riscv64_float_compare", |b| {
