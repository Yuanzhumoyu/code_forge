//! riscv64 整数指令测试（test-int feature 门控）— compile-only。
//!
//! riscv64 无本机执行；此处验证代表性整数指令组合能编译成功（产物非空），
...
#[test]
fn int_arith_chain() {
    compile_ok("riscv64_int_arith_chain", |b| {
...
#[test]
fn int_sub_const() {
    compile_ok("riscv64_int_sub_const", |b| {
...
#[test]
fn int_shift_chain() {
    compile_ok("riscv64_int_shift_chain", |b| {
...
fn int_bitwise_chain() {
    compile_ok("riscv64_int_bitwise_chain", |b| {
