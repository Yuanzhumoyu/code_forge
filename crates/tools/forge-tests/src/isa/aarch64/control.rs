//! aarch64 控制流指令测试（test-control feature 门控）— compile-only。
//!
//! aarch64 无本机执行；此处验证 icmp/select/分支组合能编译成功，
...
#[test]
fn control_icmp_eq() {
    compile_ok("aarch64_control_icmp_eq", |b| {
...
fn control_icmp_select() {
    compile_ok("aarch64_control_icmp_select", |b| {
...
fn control_two_blocks() {
    compile_ok("aarch64_control_two_blocks", |b| {
