...
//! 等分析结果。原始的支配树算法实现在 [`analysis`]，
//! 循环检测实现在 [`loop_info`]。
//!
//! 当前各优化 pass 独立构建分析结果，导致相同分析被多次重复计算。
//! `AnalysisManager` 提供懒缓存 + 失效机制。

use forge_ir::{Block, DominatorTree, Function, LoopForest, block_successors};
...
pub struct AnalysisManager {
    ...
    /// 缓存的原始支配关系。
    dominators: Option<HashMap<Block, HashSet<Block>>>,
    /// 缓存的前驱列表。
    predecessors: Option<HashMap<Block, Vec<Block>>>,
    /// 缓存的循环森林。
    loop_forest: Option<LoopForest>,
    /// 缓存的回边列表（用于向后兼容的 get_loops）。
    back_edges: Option<Vec<(Block, Block)>>,
}
...
impl AnalysisManager {
    ...
    pub fn get_dominators(&mut self, func: &Function) -> &HashMap<Block, HashSet<Block>> {
        self.dominators.get_or_insert_with(|| {
            ...
            while changed {
                changed = false;
                for (&id, blocks) in preds.iter() {
                    let mut new_dom: HashSet<Block> = if blocks.is_empty() {
                        HashSet::new()
                    } else {
                        ...
                        for p in iter {
                            intersection = intersection.intersection(&doms[p]).copied().collect();
                        }
                        intersection
                    };
                    new_dom.insert(id);
    ...
    /// 获取前驱块列表。
    pub fn get_predecessors(&mut self, func: &Function) -> &HashMap<Block, Vec<Block>> {
        self.predecessors
            .get_or_insert_with(|| func.predecessors().clone())
    }
...
mod tests {
    ...
    fn test_analysis_loop_forest_cache() {
        let sig = FunctionSignature::new(&[], &[TypeId::I32]);
        let mut b = FunctionBuilder::new("test_loop", TypeContext::new(), sig);
        let entry = b.create_block();
