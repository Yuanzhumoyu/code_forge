//! Encoder integration tests — verify `encoding` bitstring → bytes roundtrip.
//!
//! Tests cover: single-byte instructions, REX prefixes, ModR/M encoding,
//! immediate values, condition codes, SSE instructions, and multi-instruction sequences.

use codegen_lib::backend::x86_64::{X86Inst as Inst, X86Isa as Isa};
use codegen_lib::prelude::{Encoder, VReg};

fn encode(inst: &Inst) -> Result<Vec<u8>, codegen_lib::EncodeError> {
    Isa::encode(inst)
}
...
fn test_encode_push_low_reg() {
    ...
    assert_eq!(encode(&Inst::PushReg { reg: r(5) }).unwrap(), vec![0x55]);
}
...
fn test_encode_push_high_reg() {
    ...
    assert_eq!(
        encode(&Inst::PushReg { reg: r(12) }).unwrap(),
        vec![0x41, 0x54]
    );
}
...
fn test_encode_pop_low_reg() {
    // RBP(5), < 8 → 0x58 | 5 = 0x5D
    assert_eq!(encode(&Inst::PopReg { reg: r(5) }).unwrap(), vec![0x5D]);
}
...
fn test_encode_pop_high_reg() {
    ...
    assert_eq!(
        encode(&Inst::PopReg { reg: r(12) }).unwrap(),
        vec![0x41, 0x5C]
    );
}
...
fn test_encode_mov_r8_rm_low_regs() {
    ...
    assert_eq!(
        encode(&Inst::MovRRm { opsize: 64u8, dest: r(0), src: r(1) }).unwrap(),
        vec![0x48, 0x8B, 0xC1]
    );
}
...
fn test_encode_mov_rm_r8_low_regs() {
    ...
    assert_eq!(
        encode(&Inst::MovRmR { opsize: 64u8, dest: r(1), src: r(0) }).unwrap(),
        vec![0x48, 0x89, 0xC1]
    );
}
...
fn test_encode_mov_r8_rm_high_regs() {
    ...
    assert_eq!(
        encode(&Inst::MovRRm { opsize: 64u8, dest: r(8), src: r(9) }).unwrap(),
        vec![0x4D, 0x8B, 0xC1]
    );
}
...
fn test_encode_mov_r8_rm_mixed_regs() {
    ...
    assert_eq!(
        encode(&Inst::MovRRm { opsize: 64u8, dest: r(8), src: r(1) }).unwrap(),
        vec![0x4C, 0x8B, 0xC1]
    );
}
...
fn test_encode_add_rm8_r8_low_regs() {
    ...
    assert_eq!(
        encode(&Inst::AddRmR { opsize: 64u8, dest: r(2), src: r(3) }).unwrap(),
        vec![0x48, 0x01, 0xDA]
    );
}
...
fn test_encode_sub_rm8_r8_low_regs() {
    ...
    assert_eq!(
        encode(&Inst::SubRmR { opsize: 64u8, dest: r(6), src: r(7) }).unwrap(),
        vec![0x48, 0x29, 0xFE]
    );
}
...
fn test_encode_mov_reg_imm64_r9() {
    ...
    assert_eq!(
        encode(&Inst::MovRegImm64 { imm: 0xDEADBEEF, reg: r(8) }).unwrap(),
...
fn test_encode_jmp_rel32() {
    ...
    assert!(!bytes.is_empty() && bytes[0] == 0xE9,
        "JMP_REL32 should start with 0xE9, got {:?}", bytes);
}

#[test]
fn test_encode_jcc_rel32() {
    let bytes = encode(&Inst::JccRel32 { cond: 0x94, rel: 0 }).unwrap();
    assert_eq!(&bytes[..2], &[0x0F, 0x94],
        "JCC_REL32 should start with 0F 94, got {:?}", bytes);
}
...
fn test_encode_not_rm() {
    ...
    assert_eq!(
        encode(&Inst::NotRm { opsize: 64u8,  dest: r(0) }).unwrap(),
        vec![0x48, 0xF7, 0xD0]
    );
}
...
fn test_encode_xorpd_xmm() {
    let bytes = encode(&Inst::Xorpd { dest: r(0), src: r(1) }).unwrap();
...
fn test_encoded_size_ret() {
    assert_eq!(Isa::encoded_size(&Inst::Ret).unwrap(), 1);
}

#[test]
fn test_encoded_size_push_r12() {
    assert_eq!(Isa::encoded_size(&Inst::PushReg { reg: r(12) }).unwrap(), 2);
}

#[test]
fn test_encode_into_buffer() {
    let mut buf = [0u8; 16];
    let n = Isa::encode_into(&Inst::Ret, &mut buf).unwrap();
    assert_eq!(n, 1);
...
fn test_encode_into_buffer_too_small() {
    let mut buf = [0u8; 0];
    let result = Isa::encode_into(&Inst::Ret, &mut buf);
...
fn encode_seq(insts: &[Inst]) -> Result<Vec<u8>, codegen_lib::EncodeError> {
    let mut buf = Vec::new();
    for inst in insts {
        buf.extend(Isa::encode(inst)?);
    }
    Ok(buf)
}
...
fn test_encode_sequence_arithmetic() {
    ...
    let seq = &[
        Inst::AddRmR { opsize: 64u8, dest: r(2), src: r(3) },
        Inst::SubRmR { opsize: 64u8, dest: r(6), src: r(7) },
        Inst::XorRmR { opsize: 64u8, dest: r(4), src: r(5) },
    ];
    let bytes = encode_seq(seq).unwrap();
...
fn test_encode_sequence_epilogue() {
    ...
    let seq = &[
        Inst::PopReg { reg: r(15) },
        Inst::PopReg { reg: r(14) },
        Inst::PopReg { reg: r(5) },   // rbp
        Inst::Ret,
    ];
    let bytes = encode_seq(seq).unwrap();
...
fn test_encode_sequence_branch() {
    // cmp rax, rcx; je .L10
    let seq = &[
        Inst::CmpRmR { opsize: 64u8,  src1: r(0), src2: r(1) },
        Inst::JccRel32 { cond: 0x94, rel: 10 },
    ];
    let bytes = encode_seq(seq).unwrap();
...
fn test_encode_large_sequence_deterministic() {
    ...
    let bytes2 = encode_seq(&seq).unwrap();
    assert_eq!(bytes1, bytes2, "large sequence encoding should be deterministic");
