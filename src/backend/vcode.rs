...
pub struct VCodeBlockId(pub u32);
...
impl<I> VCodeBlock<I> {
    pub fn new(id: VCodeBlockId, ir_block: BlockId) -> Self {
        Self {
            id,
            ir_block,
...
pub struct VCode<I> {
    ...
    current_block: Option<VCodeBlockId>,
}
...
impl<I> VCode<I> {
    ...
    /// 创建一个新块，绑定到指定的 IR BlockId。
    pub fn create_block(&mut self, ir_block: BlockId) -> VCodeBlockId {
        let id = VCodeBlockId(self.blocks.len() as u32);
        self.blocks.push(VCodeBlock::new(id, ir_block));
        id
    }

    /// 切换到指定块，后续 `push_inst` 将向该块添加指令。
    pub fn switch_to_block(&mut self, block: VCodeBlockId) {
        assert!(
    ...
    /// 按 ID 获取块。
    pub fn block(&self, id: VCodeBlockId) -> Option<&VCodeBlock<I>> {
        self.blocks.get(id.0 as usize)
    }

    /// 按 ID 获取可变块引用。
    pub fn block_mut(&mut self, id: VCodeBlockId) -> Option<&mut VCodeBlock<I>> {
    ...
    /// 获取 IR BlockId → VCodeBlockId 的映射。
    pub fn ir_to_vcode_map(&self) -> Vec<(BlockId, VCodeBlockId)> {
