...
impl PatternMatcher {
    ...
    /// `constants` 为可选的常量池引用。
    /// 返回所有匹配的列表，按位置升序排列。
    pub fn find_all_matches(&self, insts: &[Instruction], constants: Option<&[Big]>) -> Vec<PatternMatch> {
    ...
    pub fn apply<'a>(&self, insts: &'a [Instruction], constants: Option<&[Big]>) -> Vec<(usize, &'a Instruction)> {
    ...
    pub fn register_x86_standard_patterns(&mut self) {
        ...
        self.add_pattern(Pattern {
            ...
            condition: Some(|matched, constants| {
                ...
                if let Some(imul_result) = imul_inst.result {
        ...
        self.add_pattern(Pattern {
            ...
            condition: Some(|matched, _constants| {
                ...
                if let Some(icmp_result) = icmp_inst.result {
        ...
        self.add_pattern(Pattern {
            ...
            input: vec![
                Opcode::Fmul {
                    flags: crate::ir::FastMathFlags::default(),
                },
                Opcode::Fadd {
                    flags: crate::ir::FastMathFlags::default(),
                },
            ...
            condition: Some(|matched, _constants| {
                ...
                if let Some(fmul_result) = fmul.result {
...
mod tests {
    ...
    fn make_inst(opcode: Opcode, result: Option<Value>, operands: &[Value]) -> Instruction {
        Instruction::new(
            opcode,
    ...
    fn test_pattern_matcher_empty() {
        ...
        let matches = matcher.find_all_matches(&insts, None);
    ...
    fn test_pattern_matcher_no_match() {
        let matcher = PatternMatcher::new();
        let insts = vec![
            make_inst(Opcode::Iadd, Some(Value(1)), &[Value(0), Value(2)]),
            make_inst(Opcode::Isub, Some(Value(3)), &[Value(1), Value(4)]),
        ];
        let result = matcher.apply(&insts, None);
        assert_eq!(result.len(), 2); // no patterns registered, all pass through
    }

    #[test]
    fn test_leaq_merge_pattern_detected() {
        let mut matcher = PatternMatcher::new();
        matcher.register_x86_standard_patterns();

        // Imul(base, const_scale) then Iadd(index*scale, disp_base)
        let insts = vec![
            make_inst(Opcode::Imul, Some(Value(5)), &[Value(1), Value(2)]),
            make_inst(Opcode::Iadd, Some(Value(6)), &[Value(5), Value(3)]),
            make_inst(Opcode::Nop, None, &[]),
        ];

        let matches = matcher.find_all_matches(&insts, None);
        assert!(!matches.is_empty(), "Should detect lea-merge pattern");
        assert_eq!(matches[0].pattern_name, "lea-merge-iadd-imul");
        assert_eq!(matches[0].consumed, 2);
    }

    #[test]
    fn test_cmp_select_fusion_detected() {
        let mut matcher = PatternMatcher::new();
        matcher.register_x86_standard_patterns();

        // Icmp then Select using icmp result as condition
        let icmp = Opcode::Icmp {
            cond: crate::ir::IntCC::Equal,
        };
        let select = Opcode::Select;

        let insts = vec![
            make_inst(icmp, Some(Value(10)), &[Value(1), Value(2)]),
            // Select(cond=Value(10), true=Value(3), false=Value(4))
            make_inst(select, Some(Value(11)), &[Value(10), Value(3), Value(4)]),
        ];

        let matches = matcher.find_all_matches(&insts, None);
        assert!(!matches.is_empty(), "Should detect cmp-select fusion pattern");
        assert_eq!(matches[0].pattern_name, "cmp-select-fusion");
    }

    #[test]
    fn test_fma_pattern_detected() {
        let mut matcher = PatternMatcher::new();
        matcher.register_x86_standard_patterns();

        // Fmul then Fadd where fmul result feeds fadd
        let fmul_op = Opcode::Fmul {
            flags: crate::ir::FastMathFlags::default(),
        };
        let fadd_op = Opcode::Fadd {
            flags: crate::ir::FastMathFlags::default(),
        };

        let insts = vec![
            make_inst(fmul_op, Some(Value(10)), &[Value(1), Value(2)]),
            make_inst(fadd_op, Some(Value(11)), &[Value(10), Value(3)]),
        ];

        let matches = matcher.find_all_matches(&insts, None);
        assert!(!matches.is_empty(), "Should detect mul-add fusion pattern");
        assert_eq!(matches[0].pattern_name, "mul-add-fusion");
    }

    #[test]
    fn test_fma_no_match_when_no_def_use_chain() {
        let mut matcher = PatternMatcher::new();
        matcher.register_x86_standard_patterns();

        // Fmul then Fadd, but fadd doesn't use fmul's result
        let fmul_op = Opcode::Fmul {
            flags: crate::ir::FastMathFlags::default(),
        };
        let fadd_op = Opcode::Fadd {
        ...
        let insts = vec![
            make_inst(fmul_op, Some(Value(10)), &[Value(1), Value(2)]),
            // fadd uses Value(4) and Value(5) — NOT fmul's result Value(10)
            make_inst(fadd_op, Some(Value(12)), &[Value(4), Value(5)]),
        ];

        let matches = matcher.find_all_matches(&insts, None);
        // Should not match because def-use chain is broken
        assert!(matches.is_empty(), "Should NOT match when fadd doesn't use fmul's result");
    }
}
