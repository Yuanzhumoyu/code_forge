...
macro_rules! pack {
    ($sink:expr, $bits:expr, [$(($val:expr, $width:expr)),* $(,)?]) => {{
        #[allow(unused_mut)]
        let mut _off: u8 = 0;
        $crate::backend::encode::pack_bits($sink, $bits, &[
            $({
                let o = _off;
                _off += $width;
                $crate::backend::encode::BitField::new(($val) as u64, o, $width)
            }),*
        ]);
    }};
}
...
macro_rules! pack_label {
    ($sink:expr, $bits:expr, [$(($val:expr, $width:expr)),* $(,)?], $target:expr, $reloc:expr) => {{
        #[allow(unused_mut)]
        let mut _off: u8 = 0;
        $crate::backend::encode::pack_bits_label(
            $sink, $bits,
            &[
                $({
                    let o = _off;
...
mod tests {
    ...
    fn test_pack_label_macro() {
        let mut sink = CodeSink::new();
        let target = crate::ir::BlockId(42);
        pack_label!(
            &mut sink,
            32,
