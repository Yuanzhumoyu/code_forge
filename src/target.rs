...
impl TargetConfig {
    /// 获取宿主平台的目标配置。
    pub fn host() -> Result<Self, crate::CompileError> {
    ...
    /// ```
    pub fn from_triple(triple_str: &str) -> Result<Self, crate::CompileError> {
        let triple = triple_str.parse::<Triple>().map_err(|e| {
            crate::CompileError::Unsupported(format!("invalid triple '{triple_str}': {e}"))
        })?;
        Ok(Self { triple })
    }
