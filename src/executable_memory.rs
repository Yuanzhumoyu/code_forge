...
impl ExecutableMemory {
    ...
    pub fn new(code: &[u8]) -> Result<Self, CompileError> {
        if code.is_empty() {
            return Err(CompileError::Emit("empty code".into()));
        }
    ...
    pub fn new_writable(code: &[u8]) -> Result<Self, CompileError> {
        if code.is_empty() {
            return Err(CompileError::Emit("empty code".into()));
        }
    ...
    pub unsafe fn modify<F>(&mut self, f: F) -> Result<(), CompileError>
    where
        F: FnOnce(&mut [u8]),
    {
        if self.is_sealed {
    ...
    /// 从可执行内存中获取函数指针。
    ///
    /// Returns `Err(CompileError::Emit(...))` if the offset is out of bounds.
    ///
    /// # Safety
    ///
    /// - 调用者必须确保类型 `F` 的函数签名与偏移处代码的实际调用约定匹配
    /// - 代码必须在这一偏移处开始一个有效的函数入口点
    pub unsafe fn get_fn<F>(&self, offset: usize) -> Result<F, CompileError> {
        if offset >= self.size {
    ...
    pub unsafe fn exec_once<F, R>(code: &[u8], f: impl FnOnce(F) -> R) -> Result<R, CompileError> {
...
/// 分配可读-可写内存（不可执行）。
/// W^X 安全模型的第一步。
fn alloc_rw(size: usize) -> Result<*mut u8, CompileError> {
    #[cfg(windows)]
    {
        #[allow(unsafe_code)]
        unsafe extern "system" {
        ...
        if ptr.is_null() {
            return Err(CompileError::Emit(format!(
                "VirtualAlloc failed for {size} bytes"
            )));
        }
    ...
    #[cfg(not(windows))]
    {
        ...
        if ptr as usize == usize::MAX || ptr.is_null() {
            return Err(CompileError::Emit(format!("mmap failed for {size} bytes")));
        }
...
/// 切换内存保护为可读-可执行（不可写）。
fn make_executable(ptr: *mut u8, size: usize) -> Result<(), CompileError> {
    #[cfg(windows)]
    {
        #[allow(unsafe_code)]
        unsafe extern "system" {
...
/// 临时切换回可写（用于 patching）。
fn make_writable(ptr: *mut u8, size: usize) -> Result<(), CompileError> {
    #[cfg(windows)]
    {
        #[allow(unsafe_code)]
        unsafe extern "system" {
