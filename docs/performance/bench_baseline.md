...
**v18 S2 首例迁移（2026-09-19）**：`[[templates]]` 参数化 + 自动 `ref` 别名落地后，
`isa/arm64_v12.toml` **1,125 → 731 行（−35%）**（32 条模板取代 64 个指令块 + 删除 29 条
手写 `[[aliases]]`）；arm64 生成代码 644,018 → **636,986 B**（−1.1%），`Inst::` 名字集合
逐一同（90 个）；黄金测试与 JIT 矩阵（23/175/0）不变。
