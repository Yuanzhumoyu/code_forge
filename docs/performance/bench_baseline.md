...
规格规模（`isa/*.toml`，**S0 采集时点**；S2a/S2c 迁移后的现状见本节末尾的表）：x86 3,356 行
（146 指令 + 51 族变体 + 25 别名 + 220 lowering）、riscv64 1,754 行（78 + 38 + 0 + 110）、
arm64 1,125 行（89 + 0 + 29 + 19），合计 **6,235 行**。
生成器 + 模型 15,976 行；生成面手写测试 2,952 行。

**v18 S2 三 ISA 迁移（2026-09-19）**：`[[templates]]` 参数化 + 自动 `ref` 别名落地后，
arm64 **1,125 → 731 行（−35%）**（32 条模板取代 64 个指令块 + 删 29 条手写别名）、
riscv64 **1,754 → 1,606 行（−8.4%）**（15 条模板取代 30 个 S/D 对）、x86 **3,356 → 3,324 行（−1%）**
（movzx/movsx 四条合成 1 条 + 删 2 条手写别名）。
生成代码：arm64 644,018 → **636,986 B**、x86 2,986,514 → **2,979,650 B**（riscv 1,263,470 → 1,263,918 B）；
三 ISA 的 `Inst::` 名字集合前后 **diff 为空**（90 / 117 / 198 个），x86 被迁移指令的编码臂文本
SHA-256 **逐字节相同**；黄金测试与 JIT 矩阵（x86 195/3/0、riscv 131/67/0、arm64 23/175/0）不变。

**v18 S2c 三机制统一（2026-09-19，取代上面的数字）**：`[[families]]`（14 条 / 100 变体）与
`[[aliases]]`（23 条）全删，只剩 `[[templates]]`（`body` + `rows`）+ 指令属性 `ref`。

| ISA | TOML 行数（前 → 后） | `[[instructions]]` | 模板 / 模板行 | 指令总数 |
| --- | ---: | ---: | ---: | ---: |
| arm64 | 871 → **891（+2.3%）** | 25 → 25 | 32 / 64 | 89 |
| riscv64 | 1,899 → **1,802（−5.1%）** | 48 → 48 | 19 / 68 | 116 |
...
加了数据化与组合键（`[[reloc]]`/`[[pseudo]]`/`[encoding]`/`#:schema` 行），行数与 S2c 相比回升
（这是功能增加，不是回退）：

| ISA | TOML 行数（S2c → 现在） | 说明 |
| --- | ---: | --- |
...
数字与上表**不同口径**（x86 1,865,477 B / riscv64 1,074,439 B / arm64 674,268 B，本机 2026-09-21）。
做 S8 减薄对照时**必须固定同一个采集命令**，否则对比无意义。

> **测量口径**：首轮曾测到 96.9 s / 70.6 s 的偏大值（与后台任务重叠）；上表为单独重跑的稳定值。
> 生成代码是 `proc_macro2::TokenStream::to_string()` 的**未格式化**产物，字节数比"正常格式化后的行数"
> 更能反映编译器要吃的规模。

### S8 立项度量（2026-09-21，代码基线 a33a814）

**采集命令**（固定这一条，S8 前后对照都用它）：
`cargo test -p forge-isa-dsl --test zz_tmp_s8` —— 临时用例内用
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| x86 | 1,434,080 | 1,851,847 | **841,747 (45.5%)** | 213,824 (11.5%) | 182,082 (9.8%) | 160,305 (8.7%) | 36,122 (2.0%) | 417,767 (22.6%) |
| riscv64 | 789,070 | 1,074,439 | **527,551 (49.1%)** | 68,661 (6.4%) | 58,239 (5.4%) | 113,852 (10.6%) | 20,767 (1.9%) | 285,369 (26.6%) |
| arm64 | 395,225 | 674,268 | 172,067 (25.5%) | 53,382 (7.9%) | 47,880 (7.1%) | 101,759 (15.1%) | 20,137 (3.0%) | **279,043 (41.4%)** |

（括号内占比的分母 = "全部件 + spec_tests"；表内其余数字为字节。）

**再往下归因**（把 tm-only 生成物按顶级条目切开）：tm 的 96–97% 是 `impl` 块，其中两块最大：

| ISA | `impl TargetLowering for Lowering` | `impl MachineInst for Inst` | 二者占 tm |
| --- | ---: | ---: | ---: |
| x86 | 611,679（69.8%；`Inst::` 臂 944） | 185,382（21.2%；`Inst::` 臂 1,601） | **91%** |
| riscv64 | 370,749（67.8%；475） | 116,451（21.3%；937） | **89%** |
| arm64 | 38,346（20.1%；28） | 90,996（47.7%；841） | 68% |

**`cargo check` 成本对照**（`cargo clean -p forge-codegen` 后单跑）：

| 配置 | 时间 |
| --- | ---: |
| 现状（三个发行后端 `spec_tests` 缺省 = true） | **27.4 s** |
| 三个后端临时改 `spec_tests = false` | **29.9 s**（在噪声内） |
...
### S8b 立项度量（2026-09-21；S8a 之后，按 op 臂切开 `impl TargetLowering for Lowering`）

采集：把 `parts = tm` 的生成物按 `Opcode :: X { .. } => {` 顶层臂切开，量三段——
A = 属性绑定（`let __a_*` + `__attr` 闭包）、B = `when` 分派、C = 发射序列
（`push_inst`/`map_reg_field`/立即数/临时寄存器）。

| ISA | impl 总长 | op 臂数 | A 属性样板 | B when 分派 | C 发射序列 |
| --- | ---: | ---: | ---: | ---: | ---: |
| x86 | 611,642 | 102 | **264,344（43.2%）** | ≈4,740（0.8%） | 334,020（54.6%） |
| riscv64 | 370,712 | 63 | **161,422（43.5%）** | ≈2,460（0.7%） | 202,324（54.6%） |
| arm64 | 38,309 | 10 | **21,467（56.0%）** | ≈360（0.9%） | 12,165（31.8%） |

**决定性事实**：A 段**与 op 无关**——x86 的 100 个带属性块的臂里是**同一段 2,433 B 文本**
逐字节重复（riscv 61 份、arm64 8 份；连 `__a_cond` 那一行也只有 1 种写法）。原因是
...
闭包只捕获这些值、不捕获 `ctx`，因此不挡后面 emit 里的 `&mut ctx`）。

| ISA | `tm` | 全部件 | +`spec_tests` | `无tm`（对照） |
| --- | --- | --- | --- | --- |
| x86 | 770,348 → **511,774（−258,574，−33.6%）** | 1,326,559 → **1,067,985（−19.5%）** | 1,744,326 → **1,485,752（−14.8%）** | 592,333（不变） |
| riscv64 | 479,381 → **322,753（−156,628，−32.7%）** | 720,133 → **563,505（−21.7%）** | 1,005,502 → **848,874（−15.6%）** | 261,137（不变） |
| arm64 | 141,262 → **123,176（−18,086，−12.8%）** | 344,283 → **326,197（−5.3%）** | 623,326 → **605,240（−2.9%）** | 222,776（不变） |

生成物里该块的出现次数：`let __a_rd = results . first ()` **100 → 1**（riscv 61 → 1，arm64 8 → 1），
守卫 `crates/frontend/forge-isa-dsl/tests/lowering_attrs_once.rs` 钉住"每个生成模块只出现一次"。

**累计（对照 S0 立项基线 1,434,080 / 789,070 / 395,225）**：

| ISA | S0 基线 | S8a 后 | S8b-1 后 | 累计 |
| --- | ---: | ---: | ---: | ---: |
| x86 | 1,434,080 | 1,326,559 | **1,067,985** | **−25.5%** |
| riscv64 | 789,070 | 720,133 | **563,505** | **−28.6%** |
| arm64 | 395,225 | 344,283 | **326,197** | **−17.5%** |

**还剩什么**：x86 的 C 段 334 KB（944 条发射臂，约 354 B/条）是**真正的每条规则数据**
（构造哪条 `Inst`、哪些字段、立即数、临时寄存器），要表化必须写一个通用解释器
（把"构造 Inst + 字段绑定 + clobber"变成数据 + 宿主循环）——收益上限 ~250 KB（x86），
但改动覆盖 `codegen/lowering.rs` 的发射路径与宿主 lowering 面，风险高，**未做**，仍待拍板。

### S8c 落地度量（2026-09-21；同树 A/B，只加 `fold_short_forms` 短名折叠）

**立项依据**（把 notm 侧按 `;` 分隔切开统计重复语句，口径：重复 ≥8 次且长 ≥40 B 的块）：
x86 80 种块共 197 KB（占 notm 33%）、riscv 26 种 80 KB（31%）、arm64 24 种 53 KB（24%）——
其中出现最多的长文本是**两条机械可折叠**的写法：

| 长形 | x86 处数 | riscv | arm64 | 短名 |
| --- | ---: | ---: | ---: | --- |
| `Reg :: from_index (…)` / `< Reg as forge_ir :: PhysReg > :: from_index (…)` | 1,696 / — | 1,406 | 252 | `__ph (…)` |
| `forge_ir :: RegClass` | 1,381 | 1,117 | 127 | `__RC` |
| `< Reg as forge_ir :: PhysReg > :: to_index (…)` | — | 273 | 192 | `__ti (…)` |

**做法**：生成器最后一道 token 后处理里折叠（`forge-isa-dsl/src/lib.rs::fold_short_forms`，
在 `rewrite_path_roots` **之前**跑），短名定义（`type __RC = forge_ir::RegClass;` +
`fn __ph/__ti`）发射在模块开头、折叠**之后**，因此自己走正常路径改写（跨 crate 生成也对）。
语义零变化：`__ph`/`__ti` 就是 `from_index`/`to_index` 的 `#[inline]` 薄封装，`__RC` 是类型别名。

| ISA | 全部件 | `tm` | `无tm` | +`spec_tests` |
| --- | --- | --- | --- | --- |
| x86 | 1,067,985 → **1,009,513（−58,472，−5.5%）** | 511,774 → **467,395（−8.7%）** | 592,333 → **576,413（−2.7%）** | 1,485,752 → **1,337,012（−10.0%）** |
| riscv64 | 563,505 → **506,886（−56,619，−10.0%）** | 322,753 → **288,793（−10.5%）** | 261,137 → **237,771（−9.0%）** | 848,874 → **720,995（−15.1%）** |
| arm64 | 326,197 → **307,685（−18,512，−5.7%）** | 123,176 → **120,648（−2.1%）** | 222,776 → **206,101（−7.5%）** | 605,240 → **521,064（−13.9%）** |

（`+spec_tests` 降幅更大：生成期自测里这类构造最密；非测试构建的编译输入看"全部件"。）

**三步累计（对照 S0 立项基线 1,434,080 / 789,070 / 395,225）**：

| ISA | S0 基线 | S8a 后 | S8b-1 后 | S8c 后 | 累计 |
| --- | ---: | ---: | ---: | ---: | ---: |
| x86 | 1,434,080 | 1,326,559 | 1,067,985 | **1,009,513** | **−29.6%** |
| riscv64 | 789,070 | 720,133 | 563,505 | **506,886** | **−35.8%** |
...
S8a/S8b-1/S8c 之后，lowering 里剩下的就是**每条规则各自的发射体**。按"发射体是否只由
`push_inst` + `map_reg_field` 构成"切开（口径：`let __cc : u8 = 0 ;` … `Ok (__pack)` 为一条发射体；
**均匀体** = `Inst` 构造里只有 `__ph(…)` 占位与字段名，无立即数/内存/标签/临时寄存器/`ctx` 调用）：

| ISA | 发射体 | 均匀体 | 复杂体（要解释器） |
| --- | ---: | ---: | ---: |
| x86 | 285 条 / 243,429 B（impl 的 78%） | 70 条 / 21,861 B（**9.0%**） | 215 条 / 221,568 B（全部件的 **21.9%**） |
| riscv64 | 110 条 / 154,608 B（85%） | 57 条 / 20,558 B（**13.3%**） | 53 条 / 134,050 B（26.4%） |
| arm64 | 19 条 / 8,772 B（47%） | 15 条 / 5,672 B（**64.7%**） | 4 条 / 3,100 B（1.0%） |

**只表化均匀体（S8b-2a，原计划的"最小子集试水"）**：换成"行表 + 专用构造器 + 通用发射器"
后按 70 B/行 + 70 B/构造器 + 900 B 驱动器估算，净回收 ≈ **x86 12,491 B（1.2%）/ riscv 12,448 B
（2.5%）/ arm64 2,672 B（0.9%）** —— 复杂度与"逐规则等价验证"的成本远大于收益，**不做**。

**连复杂体一起表化（S8b-2 全体）**：上限 221 KB（x86，全部件的 21.9%），但那些发射体里
是"立即数怎么取、内存操作数怎么组装、标签/临时寄存器怎么分配、`ctx` 怎么改"这些**异质逻辑**
（x86 平均 1,030 B/条），要变成数据就必须写一个**通用解释器**（含谓词判定、立即数/内存/标签/
临时寄存器的数据表达），等于把后端降级路径改成解释器——风险与工作量超过 S8a + S8b-1 + S8c
三项之和，而收益上限只有 21.9%。**按"以度量决定"判据：S8 就此收尾。**

三步落地的最终账（对照 S0 立项基线，`全部件`，采集命令同 S8a 节）：

| ISA | S0 基线 | S8a | S8b-1 | S8c | 累计 | 生成期自测（`+spec_tests`）累计 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| x86 | 1,434,080 | 1,326,559 | 1,067,985 | **1,009,513** | **−29.6%** | 1,851,847 → 1,337,012（−27.8%） |
| riscv64 | 789,070 | 720,133 | 563,505 | **506,886** | **−35.8%** | 1,074,439 → 720,995（−32.9%） |
| arm64 | 395,225 | 344,283 | 326,197 | **307,685** | **−22.1%** | 674,268 → 521,064（−22.7%） |
...
① 谓词求值走 `__attr(name) -> Option<i64>` 的运行时 `match name { "rd" => __a_rd, … }`
字符串分派（属性名在**生成期**就已知，这层转换毫无意义）；
② 9 个属性在**每次 `lower_inst` 调用**里全算一遍，而 `when` 里用不到的属性（例如
`elem`/`iconst`/`cond`）对那些 op 完全是白算。另外发现一处死代码：不用 `{cc}` 的规则
也发射 `let __cc: u8 = 0;`（x86 285 处，永远读不到）。

**改法**：谓词名在生成期解析成具体方法（`attr_expr`），运行时只剩
`__ac . <属性> (&*ctx)`：
`__AC<'x>` 存 `done` 位图 + `v: [Option<i64>; 9]` + `op`/`args`/`results` 三个共享借用
（**不借 `ctx`**，所以后面的 `&mut ctx` 不受影响）；9 个属性各是
`#[inline] fn <名>(&mut self, ctx: &LowerCtx) -> Option<i64>`：**没算过才算**，
算过就复用——旧实现"每次调用每属性最多算一次"的语义因此保持不变，只是从"全部预计算"
变成"按需"。`{cc}` 绑定也改成只在真用 `{cc}` 的规则里发射。

| ISA | 全部件 | `tm` | +`spec_tests` | `let __cc: u8 = 0;` 处数 | `__attr` 名字分派 |
| --- | --- | --- | --- | ---: | ---: |
| x86 | 1,009,513 → **1,009,156（−357）** | 467,395 → 467,038 | 1,337,012 → 1,336,655 | 285 → **0** | 有 → **无** |
| riscv64 | 506,886 → 508,229（+1,343） | 288,793 → 290,136 | 720,995 → 722,338 | 110 → **0** | 有 → **无** |
| arm64 | 307,685 → 310,464（+2,779） | 120,648 → 123,427 | 521,064 → 523,843 | 19 → **0** | 有 → **无** |

**尺寸基本持平**（x86 −0.04%，riscv +0.26%，arm64 +0.9%）：省掉的是 285/110/19 处死
`__cc` 绑定与 `__attr` 闭包，多出来的是 9 个方法（各带签名）与变长的调用点
（`__ac . rs1_width (&*ctx)` 比 `__attr ("rs1_width")` 长几个字节；385 个调用点）；
换来的是**每次调用的属性求值数从固定 9 降到实际引用数**（多数 op 是 0~2 个），
以及每次谓词求值少一次 9 臂字符串 match。

**四步累计（对照 S0 立项基线 1,434,080 / 789,070 / 395,225）**：

| ISA | S0 基线 | S8a | S8b-1 | S8c | S8d | 累计 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| x86 | 1,434,080 | 1,326,559 | 1,067,985 | 1,009,513 | **1,009,156** | **−29.6%** |
| riscv64 | 789,070 | 720,133 | 563,505 | 506,886 | **508,229** | **−35.6%** |
| arm64 | 395,225 | 344,283 | 326,197 | 307,685 | **310,464** | **−21.4%** |

守卫：`crates/frontend/forge-isa-dsl/tests/lowering_attrs_once.rs`（生成物里
`__attr`/`__attr_core` 必须为 0；`__AC` / `__AC::new` / 9 个属性方法各只发射一次；
谓词必须经 `__ac . <属性>` 取；`let __cc: u8 = 0;` 必须为 0）＋单测
`core_attr_guard_uses_helper_directly`（直接展开成属性方法 + 9 个助手存在）、
`cc_binding_only_emitted_when_used`（不用 `{cc}` 不发射 `__cc`）、
`derive_attr_is_usable_in_when`（派生属性在生成期展开）。

## ir_binary（2026-09-19 首次采集；2026-09-19 补 v2 压缩对照）

命令：`cargo bench -p forge-ir --bench ir_binary`（criterion 默认采样）。
夹具：中等模块（884 B 文本；globals 含常量表达式/聚合、metadata、命名类型、两个函数），2 函数。

### v1（未压缩）与 v2（段体压缩）对照

| 版本 | 字节流 | encode (µs, 中位数) | decode (µs, 中位数) |
| --- | ---: | ---: | ---: |
| v1（`IR_FORMAT_VERSION = 1`，不压缩） | 1335 B（1.51×） | 21.9（19.3–24.9） | 77.2（75.8–78.6） |
| v2（`IR_FORMAT_VERSION = 2`，压缩） | **958 B（1.08×）** | **22.5**（22.1 / 30.5 / 22.5，3 次运行） | **42.9**（43.5 / 42.9 / 32.0，3 次运行） |

- **encode：压缩的成本落在噪声内**（v1 21.9 µs → v2 三次运行中位数 22.5 µs，约 +3%）。
  这是**优化后**的结果：最初每段新建一张 256 KiB 哈希表，同一夹具实测 **184.1 µs**；
  改成哈希 scratch 跨段复用 + 表长随输入自适应后回到基线量级。再把表长下限从 12 降到
  10 无可测收益 ⇒ 剩余成本在 LZ77 搜索本身，且对 1 KB 级段体可以忽略。
- **decode −44%**：字节少了（1335 → 958 B）⇒ 读侧要走的游标/切片/校验也少了；重建 arena
  与重算 use-lists 的工作量不变。压缩不改变解码的 fail-closed 纪律。
- **本机计时波动大，单点数字不作回归判据**：同一版代码、同一测量点在不同会话可差 ~1.7×
  （v2 encode 曾测得 45.2 µs 与 26.0 µs，两次都是在后台有编译/扫描活动时采集）。
  因此本节一律给"方向 + 区间 / 多次运行"，v1 的 21.9/77.2 也按同口径（`--warm-up-time 1
  --measurement-time 2`）与 v2 的三次运行对照。

尺寸基线（198 个 LLVM `test/Assembler` 正向用例，`tests/binary_module.rs` 打印）：

| 版本 | 源码文本 | 字节流合计 | 比值 | 最大单例 | 平均 |
| --- | ---: | ---: | ---: | ---: | ---: |
| v1（不压缩） | 408,810 B | 241,440 B | 0.59× | 31,038 B | 1,219 B |
| v2（压缩） | 408,810 B | **121,257 B** | **0.30×** | **8,482 B** | **612 B** |

压缩把字节流再砍掉一半（241,440 → 121,257 B，×0.50）；对源码文本的整体比值
0.59× → **0.30×**。最大单例 `auto_upgrade_nvvm_intrinsics.ll` 31,038 → 8,482 B
（×0.27，重复度高的模块收益更大）。

### 文件级 IR 缓存（`IrCache`）：两组尺度、结论相反

命令：`cargo bench -p forge-ir --bench ir_binary -- ir_binary_cache`。
两个夹具：中等模块（884 B 文本 → 958 B 字节流）与语料里文本最大的
`auto_upgrade_nvvm_intrinsics.ll`（43,207 B 文本 → 8,482 B 字节流）。
`cache_hit` = 读条目 + 反序列化；拆出 `read_entry`（只读文件）与 `decode_entry`
（只在内存里解码）两项，好说清"省在哪、亏在哪"。

| 测量点（µs，中位数） | 中等模块 884 B | 大模块 43,207 B |
| --- | ---: | ---: |
| `parse_text`（文本 → 模块） | 77.3 / 94.1 / 84.3 | 1834.7 / 2319.9 / 2192.0 |
| `cache_hit`（读条目 + 解码） | **126.2 / 114.3 / 195.0** | **723.2 / 1145.9 / 946.1** |
| `read_entry` | 75.1 / 203.3 / 81.7 | 121.1 / 205.8 / 127.8 |
| `decode_entry` | 32.4 / 40.6 / 40.2 | 548.5 / 624.7 / 683.8 |

（每格三次运行的实测值，按采集顺序；本机计时波动可达 ~1.7×，故给全部三次而不是单点。
三次运行中 `cache_hit` 与 `read_entry + decode_entry` 之和不严格相等——两次测量的分配/
释放模式不同，属测量口径差异。）

**结论（按尺度分）**：

- **小模块（≤ ~1 KB 文本）：缓存是亏的**（三次运行 126/114/195 µs vs 解析 77/94/84 µs，
  慢 1.2–2.3×）——命中要付**文件读取的固定成本**（本机 75–204 µs / 次，Windows 上含杀软
  扫描），而解析这么小的文本比它便宜。
- **大模块（几十 KB 文本）：缓存是赚的**（723/1146/946 µs vs 解析 1835/2320/2192 µs，
  快 **2.0–2.5×**，每个模块省 ~0.9–1.4 ms）。
- 盈亏平衡点 ≈ "文本解析成本 > 读文件 + 解码成本"：本机上介于 1 KB 与 43 KB 文本之间；
  由于文件读取成本随机器/杀软/文件系统波动，**缓存是否划算必须按目标环境实测**。

### 端到端对照：示例跑完整语料目录（452 个 `.ll`，119 个可解析）

命令：`cargo run -p forge-ir --example ir_binary --release -- --cache target/example-cache <452 个 .ll>`
（从 `crates/foundation/forge-ir/tests/llvm_assembler_cases` 目录内以相对路径运行，
避免 Windows 命令行长度上限）。

| 模式 | 第 1 次 | 第 2 次 |
| --- | ---: | ---: |
| 不走缓存（`--cache` 缺省） | 272 ms | 155 ms |
| 走缓存（预置后全命中 119/452） | 179 ms | 194 ms |

**结论：在这个夹具上缓存没有可测收益**（差异在噪声内），原因有三，都值得记住：

1. 452 个文件里 **332 个本来就解析失败**（负向语料），失败路径没有可缓存的东西；
2. 可解析的 119 个**都很小**（合计 165 KB 文本、最大字节流 2 KB）——落在上一张表的
   "小模块是亏的"区间；
3. 示例对每个成功文件还要 encode + decode + 逐函数校验 + 文本对照 + 幂等对照，这些
   固定成本与缓存无关。

因此本仓库的用法约定是：**缓存机制照做（它在大模块/重复处理的场景确实省时间），但
不在"正确性测试"里用它**——测试要覆盖解析器本身，缓存会把这一步短路掉；基准与夹具
运行器按需显式开启。
