...
汇编期的**文本级**多指令展开（`.equ`/`.macro` 的结构化兄弟）：不碰编码器、不碰 lowering。
...
汇编 `li x10, 0x1234` 时：按**顶层逗号**切分实参（`()`/`[]` 内的逗号不算，故
`li x1, (a + b)`、内存操作数 `[x2, #4]` 都能写）→ 逐行把 `{参数}` 换成实参文本 →
展开出的每一行交给**同一套汇编器**装配。

| 规则 | 说明 |
| --- | --- |
| 算术 | emit 行里的表达式由**既有表达式求值器**求值（`+ - * / % << >> & \| ^ ~`、括号、`.equ` 符号）——本能力不引入新的表达式语言 |
| 括号 | 实参是**文本**：`{imm}` 参与算术时请自己加括号（`({imm} + 0x800)`），否则会撞上运算符优先级 |
| 嵌套 | emit 行可以是**别的伪指令**（递归展开，深度上限 16 防自引用成环） |
| 单条 API | `assemble()` 只接受展开成 **1 条**指令的伪指令；多条要用 `TargetAssembler::parse_insts`（整段汇编），错误消息会指引 |
| 校验 | 名字非空/唯一/**不得与指令助记符重名**；`params` 非空唯一；emit 行首词必须是指令助记符或别的伪指令名；`{…}` 必须是声明的参数，且每个参数都得用到 |

未实现（如实记录，不做半成品）：按谓词分派同名多条（汇编期没有 IR 属性可判，
需要多种展开时用不同的伪指令名）；`pseudo_fold`（反汇编折叠回伪指令）——
那是指令级模式识别，与 `[[pattern]]` 是同一类问题，另立片。

riscv 的 `li` 就是本能力的实际用例：`li x10, 0x1234` 展开成 `lui x10, 1`（0x00001537）
与 `addi x10, x10, 0x234`（0x23450513）两条指令，与 lowering 里 `Iconst` 用的
`{iconst_hi20}`/`{iconst_lo12}` 完全同一套算术。
...
| `iconst` | 当前指令常量池解析出的**真值**（signed i64；与 `imm0` 的"池索引"区分） |
| `[[derive]]` 声明的名字 | 见下节（值 = 1/0） |

### `[[derive]]` — 给谓词起名字（v18 S3f）

同一个条件在多条规则里重复时，用 `[[derive]]` 起个名字（一次声明、多处引用）：
...
- `expr` 就是上面那套结构化谓词（同一份语法与校验）；派生属性取值为 **1（真）/0（假）**，
  所以用 `eq`/`ne`/`in` 引用它；
- **派生只能引用核心属性**（不支持派生引用派生）：属性名出现在值位置的代换语义
  （`eq = ["a", 0]` 该展开成什么？）并不唯一，故直接拒绝并给出提示；
- 名字**不得与核心属性重名**（否则静默遮蔽）；名字可以出现在 `vary` 里——与核心属性
  一样自动追加 `eq = [名字, 行值]` 到 `when`；
- 依赖属性缺失时该派生为**假**（与核心属性"未知即假"一致）；展开在解析期完成，
  生成期的谓词判定逻辑一行未改（没有 `[[derive]]` 时生成的属性表与引入前逐字相同）。
...
| `SPEC_INSTS` | 全部指令名（`[[templates]]` 展开后）——派生枚举器的覆盖清点用 |
| `all_insts()` | **谱内派生的代表实例**（v19 V3d）：`Vec<(名字, Inst)>`，见下 |

**派生枚举器 `all_insts()`（v19 V3d）**：宿主侧"全指令编解码往返"不必再手抄一份
`all_insts()`——生成物按谱派生即可（取值规则与生成期自测**同源**，见
`sample_operands`）：
...
因为视图与风味）。守卫 `crates/backend/forge-codegen/src/isa_roundtrip_guard.rs`：三谱
**全部**条目 `encode → decode → encode` 字节闭环 + `SPEC_INSTS` 里每条指令都在枚举器里
（漏一条就报哪条）。**为什么值得**：x86 曾手抄 650 行 `all_insts()`，谱加指令时那份清单
不会自动跟上（漏测且没人发现）——换成本枚举器后三个 ISA 测试文件从 1225 行降到 **557 行**
（相对 V3 前的 1832 行 = **−69.6%**）。`decode(encode(x)) == x` **不**在这条守卫里：
别名撞车时按声明序首匹配，本就不成立——那条断言仍由各 ISA 测试里的"规范指令"小清单守着。
