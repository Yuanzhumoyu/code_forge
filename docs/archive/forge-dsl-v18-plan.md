...
- **S5 已落地（2026-09-21，范围按实测收窄）**：新增 `[[lowering]].op` **名单**
  （`op = ["Copy", "Uextend", "Freeze"]`：一条规则服务多个同类 op，解析期展开成逐 op 规则，
  名单序 = 展开序）。迁移随之落地（数字都是**声明数**；`forge-isa insts` 报的是展开后的
  执行规则数，因此略有不同）：x86 同 op 冗余 `when` 折叠 13 组 + 跨 op 归并 6 组
  （220 → **197** 条声明 / −10.5%，展开后执行规则 299 → 286；TOML 3,693 → 3,518 行）；
  riscv 3 组（110 → **106** 条声明，−3.6%；展开后仍是 110——跨 op 归并只是把同一份序列
  写进一条规则）；arm64 无变化。
  **等价证据**：`target/s5b_verify3.py` 对三份谱逐 op 在**同一属性空间**上比较判定函数
  （属性取值 ∪ {v±1, 0, 哨兵}，按 (priority 降, 叶子数降, 声明序升) 取首条命中）——
  x86 100 个 op / 2,155 个输入格、riscv 61/352、arm64 8/61 **逐格相同**；随后
  `cargo test -p forge-codegen`（27 个测试二进制，含全部黄金字节与 417 条指令的规格自测）
  与三架构 JIT 矩阵全绿。
  **按实测收窄的范围（不做，含理由与数字）**：
  ① `lowering.emit` 结构化表（`inst`/`let`/`select`/`switch`）——族式 op 的规则在**多个属性上**
  同时分派（Vextract 24 条：rs1_width × elem × imm0 区间），表格化在规则数上只值 ~5%
  （x86 220 → 209 上限），而 `vary` 已覆盖"同一模板按行代入"的形态（V512 的 15 lane 就是
  `vary` 一行）；
  ② `[[sequences]]` 共享序列——跨 op 的长序列复用实测为 **0**（x86 ≥3 条指令的规则 88 条、
  77 个不同序列，无一与别的 op 相同）。
  两条都会引入**第二个参数化机制**（用户明确反对机制增殖：`[[templates]]`/`[[families]]`/
...
- **S8b 立项度量 + S8b-1 已落地（2026-09-21）**：先按"以度量决定"把
  ...
  **仍未做（待拍板）**：S8b-2——x86 的 C 段 334 KB（944 条发射臂）是真正的每条规则数据，
  ...
  但风险高。
- **S8c 已落地（2026-09-21）**：生成物**短名折叠**（纯等价 token 改写）。先按 `;` 分隔
  切块统计了非 tm 侧的重复语句（≥8 次且 ≥40 B：x86 197 KB / riscv 80 KB / arm64 53 KB，
  占各 notm 的 33%/31%/24%），其中最密集的是两条机械可折叠的长形：
  `Reg :: from_index (…)` / `< Reg as forge_ir :: PhysReg > :: from_index (…)`（x86 1,696 处）、
  `forge_ir :: RegClass`（x86 1,381 处）、`< … > :: to_index (…)`（riscv 273 / arm64 192）。
  改法 = 生成器最后一道 token 后处理 `fold_short_forms`（跑在 `rewrite_path_roots` 之前）
  把它们折成 `__ph (…)` / `__RC` / `__ti (…)`，短名定义（`type __RC = …` + `fn __ph/__ti`）
  发射在模块开头、折叠之后，因此自己仍走正常路径改写（跨 crate 生成同样正确）。

  实测（同树 A/B，只加折叠）：

  | ISA | 全部件 | `tm` | `无tm` | +`spec_tests` |
  | --- | --- | --- | --- | --- |
  | x86 | 1,067,985 → **1,009,513（−5.5%）** | 511,774 → **467,395（−8.7%）** | 592,333 → **576,413（−2.7%）** | 1,485,752 → **1,337,012（−10.0%）** |
  | riscv64 | 563,505 → **506,886（−10.0%）** | 322,753 → **288,793（−10.5%）** | 261,137 → **237,771（−9.0%）** | 848,874 → **720,995（−15.1%）** |
  | arm64 | 326,197 → **307,685（−5.7%）** | 123,176 → **120,648（−2.1%）** | 222,776 → **206,101（−7.5%）** | 605,240 → **521,064（−13.9%）** |

  三步累计对照 S0 立项基线：x86 1,434,080 → **1,009,513（−29.6%）**、riscv 789,070 →
  **506,886（−35.8%）**、arm64 395,225 → **307,685（−22.1%）**。
  守卫 = `crates/frontend/forge-isa-dsl/tests/short_form_fold.rs`（生成主体里三种长形一处不剩、
  短名定义各一次、`krate = …` 时短名定义里的路径也改写）。
- **S8b-2 立项度量 → 判定不做（2026-09-21，S8 就此收尾）**：把 lowering 里剩下的"每条规则
  各自的发射体"切开（均匀体 = 只由 `push_inst` + `map_reg_field` 构成、`Inst` 构造里只有
  `__ph(…)` 占位；复杂体 = 含立即数/内存/标签/临时寄存器/`ctx` 调用）：

  | ISA | 发射体 | 均匀体 | 复杂体 |
  | --- | ---: | ---: | ---: |
  | x86 | 285 条 / 243,429 B | 70 条 / 21,861 B（9.0%） | 215 条 / 221,568 B（全部件的 21.9%） |
  | riscv64 | 110 条 / 154,608 B | 57 条 / 20,558 B（13.3%） | 53 条 / 134,050 B（26.4%） |
  | arm64 | 19 条 / 8,772 B | 15 条 / 5,672 B（64.7%） | 4 条 / 3,100 B（1.0%） |

  - **只表化均匀体**（原计划的"最小子集试水"）：净回收 ≈ **x86 12,491 B（1.2%）**、
    riscv 12,448 B（2.5%）、arm64 2,672 B（0.9%）——不值得，**不做**。
  - **连复杂体一起表化**：上限 221 KB（x86，21.9%），但要为"立即数/内存/标签/临时寄存器/
    `ctx` 修改"这些异质逻辑写通用解释器（x86 平均 1,030 B/条发射体），风险与工作量超过
    S8a + S8b-1 + S8c 之和而收益上限只有 21.9%——按"以度量决定"，**不做**。
  - 三步落地账（对照 S0 基线 `全部件`）：x86 1,434,080 → **1,009,513（−29.6%）**、
    riscv 789,070 → **506,886（−35.8%）**、arm64 395,225 → **307,685（−22.1%）**；
    生成期自测那部分（`+spec_tests`）分别 −27.8% / −32.9% / −22.7%。
...
- **S5c 已落地（2026-09-21）**：`[[pattern]]` 与 `[[lowering]]` **统一裁决序**——
  ...
  供 codegen（生成 `__PATTERNS`）与校验器**共读一份**（此前排序只写在 codegen 里）。
  同片新增**死模式检测**：匹配树结构相同、且裁决序在前者的 `when` 覆盖后者 → 编译期报错
  并点出两个 `[[pattern]]` 下标（不同树之间不做覆盖推断——保守，宁可漏报不误报）。
  x86 现有两条 `[[pattern]]`（`Fadd(Fmul(a,b),c)` 的 elem 1/2）都没有 `priority`，
  新旧排序键等价 ⇒ 生成物不变；新增 4 条单测（priority 排序、死模式、完全重复、不同树不报）。
