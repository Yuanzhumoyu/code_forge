...
（无 `::error::`）⇒ 修复后该 job 恢复绿。**后续**：#59（docs）与 #60（补守卫）同样全绿
⇒ 修复后该用例**连续 3 次通过**（修复前在 #51/#57 红，即 6 次观察中 2 次失败）。
**诚实的限定**：原缺陷是"高半区栈残留"造成的**偶发**，连续通过本身不足以终局证明"永不复发"；
但根因是确定性缺陷（8B 搬运 + 32B 槽 + 类宽 32），三块都已由确定性守卫覆盖（上表，修复前必红），
且新守卫 `test_v512_byref_caller_copy_is_64b_generation` 直接锁住 #57 的调用方路径。
