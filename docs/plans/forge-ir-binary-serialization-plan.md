...
offset 0   ┌ magic：8 字节 ASCII "FORGEIR\0"
           ...
           ├ producer：字符串表引用（varint 索引；内容形如 "forge-ir 0.1.0"）
...
| `0x01` | STRINGS | 字符串表（`num` + 各串长度 + 串字节拼接） | 无 |
...
| 字符串 / `ImmStr` | varint 索引（`0` = 空串） | 走 STRINGS 段，绝不裸写长度+字节 |
...
```text
num: varint
lengths: num × varint（第 0 项恒为 0，即空串）
bytes: 各串字节按顺序拼接（UTF-8）
```

去重按首次出现顺序（写侧用 `HashMap<ImmStr, u32>` **仅作写入期索引**，输出顺序由
首次出现决定 ⇒ 字节流仍确定）。读侧重建 `StringPool`，并校验：`num ≥ 1`、总字节数与
剩余长度一致、每段是合法 UTF-8（非法 → `Err`）。
...
├── format.rs   # 段 id/常量/magic/头部读写（唯一"格式事实源"）
├── writer.rs   # Cursor 写入侧原语 + 各段 writer
└── reader.rs   # Cursor 读取侧原语 + 各段 reader（两遍函数解码）
```
