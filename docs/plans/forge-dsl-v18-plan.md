...
**剩余**：riscv（16 对）与 x86（SSE 族 + 宽度变体）迁移，以及删除 `[[families]]`/`[[aliases]]`
两个旧节（v18 破坏性收口）。
...
### 12.7 S2 首例迁移（arm64）实测

`isa/arm64_v12.toml`：**1,125 → 731 行（−35%）**。

| 指标 | 迁移前 | 迁移后 |
| --- | ---: | ---: |
| TOML 行数 | 1,125 | **731** |
| `[[instructions]]` 块 | 89 | 25 |
| `[[templates]]` 条 | 0 | **32** |
| `[[aliases]]` 条 | 29 | **0**（由模板 `ref` 自动派生） |
| arm64 生成代码 | 644,018 B | 636,986 B（−1.1%） |
| 生成代码 `Inst::` 名字集合 | 90 | 90（**前后 diff 为空**） |
| 黄金测试 / JIT 矩阵 | 12 + 6 通过 / 23-175-0 | 同左（不变） |
