...
| **S8** 生成物减薄（可选） | 静态逻辑下沉 `forge-codegen::runtime`，生成物只留表 + 分派 | **S8a ✅ / S8b-1 ✅ / S8c ✅ 已落地**：`MachineInst` 8 个方法 → `__SHAPES` 形状表；谓词属性块从每个 op 臂移出；生成物短名折叠（`__ph`/`__ti`/`__RC`）。累计 vs S0 基线：x86 −29.6% / riscv −35.8% / arm64 −22.1%；S8b-2（发射序列表化，需解释器）待定 | 以 S0 基线对比；黄金值与矩阵不变 | 按度量决定 |
