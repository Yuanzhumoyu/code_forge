...
- **C 段（lowering 发射序列）确实是生成物编译时间的主因**：x86 的 C 段 315 KB（占全件 20.3%）
  换来全量重检 49.70 s 里的 **43.30 s（87.1%）**——这修正了 V0 期"冷启动由 rustc 解析 1.3 MB
  生成物主导"的粗判断（解析只占零头，贵在 197 条规则的 typecheck/borrowck）；riscv64 53.5%、
  arm64 19.0%。
- **但表化的 net token 收益不够**：任何表示都要存 TOML 里那 42.7 KB 规则数据，而生成物 C 段
  是它的 7.4 倍；换成"数据表 + 通用解释器"乐观净省 265 KB / 悲观 170 KB ⇒ **全件口径
  11.0%–17.1%（中位 ~14%，未过 15%）**，只有"不含 spec_tests 件"口径（16.8%–26.1%）过线。
- **日常路径不受影响**：改一个谱的 mtime（build script 重跑、生成物内容不变）只需 **9.05 s**
  ——rustc 按内容指纹跳过重检；87% 只在生成物内容真变或冷构建时兑现。
- **重启触发条件**（文档 §6）：冷构建超预算 / 需要按扩展分包生成 lowering / 新 ISA 的 C 段
  占比 ≥35%。**若只压冷构建时间**，先试低风险中间方案（规则体去重抽 helper、`parts` 不含
  `tm`、`spec_tests` 只在 test 展开），不必上解释器。
...
- **度量先行**（这是选它的理由）：三个 ISA 测试文件剩下的 1225 行里，x86 `x86_v12_tests.rs`
  ...
  降级为"将来确实需要时再说"（本计划不再把它当 V3 的收尾项）。
- **实现**：`gen_spec_tests` 里新抽 `sample_operands`（操作数取值 + 字段原样断言 + imm 槽 +
  内存槽的类），生成期自测与**新枚举器**共用同一份取值规则——两处各写一遍必然漂移。
  新 `gen_all_insts` 按谱发射 `pub(crate) fn all_insts() -> Vec<(&'static str, Inst)>`
  （`#[cfg(test)]` 下，宿主成品零成本）+ `SPEC_INSTS`（指令名清单，覆盖清点用）：
  每条指令 × 宽度视图一条 + 每个 imm 槽 `lo`/`hi` + 每个 mem 槽 `disp=8`/`disp=-8`/
  `index+scale=4`（自测基线只用 `disp=0`，手抄清单里那些 `[RAX+8]`/`[RAX+RBX*4]` 由这三条替代）。
- **实测**（2026-09-24）：枚举器条目 **x86 602 / riscv64 327 / arm64 332**（远超指令总数
...
- **行数账目**：x86 880 → 216、riscv64 190 → 190、arm64 151 → 151（后两者本轮不变）；
  三文件合计 **1225 → 557 行**，相对 V3 前的 1832 行 = **−69.6%**（原目标 −≥50% 达成）。
  剩余 557 行的构成：别名/规范实例小清单（`decode_identity_canonical` 等）、ISA 数据查询
  （类表）、ABI/JIT 集成断言——**都不是"手抄可生成的数据"**。
...
- 新集成测试 `crates/frontend/forge-isa-dsl/tests/determinism.rs`：同参数展开**两次**断言
  **token 文本逐字节相同**，并断言文件名恒定（同参数同名、`spec_tests` 变体不同名——
  S10d 的不变量）；覆盖三份发行谱、7 个可独立展开的夹具（含 `include` 多文件谱）与
  `parts = ["encode"]` 变体。实测 **3 条用例全绿**（1.6 s）。
- 动机：生成器里"按名字查表"的路径很多（位域、模板展开、`vary` 行、寄存器组），一旦某张表
  换成 `HashMap` 的迭代序、或引入时间戳/指针地址，生成物就会在两次构建之间抖动——后果是
  缓存失效（每轮重编）、"我这边能编过、CI 编不过"，而 `cargo test` 仍可能全绿。
...
- **顺带说明（含一处自我更正）**：本轮把 `report::validate_file_opts` / `validate_opts` 的第二
  参数从 `bool` 换成 `&RunOpts`（`strict_overlap` 成为它的一位，与新的 `params` 同处），
  `tests/strict_overlap.rs` 的调用点随之同步。**更正**：一度把这里的编译错误当作"V6b 遗留的
  坏测试"，核对 `68e3a92` 后确认当时签名是 `(path, strict_overlap: bool)`、那个测试是可编译的
  ——错误由本轮重构引入并当场修掉，V6b 的清单快照此前确实在跑（66 行的守卫）。
