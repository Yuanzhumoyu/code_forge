...
### S3（切片）：类型读路径纪律 —— `&TypeStore` 显式传参（display 落地，2026-09-16）

余项"去 `RwLock` + `TypeStore` 显式传参"之前一直没动，是因为**真"去锁"在当前结构下
做不到**：读者要拿 `&TypeStore`（稳定地址），而 intern 会 `push` 到同一个 `Vec`——
不上 unsafe、不加依赖（如 `arc-swap`）就不可能同时成立。写成设计后把它拆成两步，
本切片做第一步与度量：

**① 语义**：`RwLock` **不可重入** ⇒ 读锁存活期间再 `borrow_mut()`（intern 新类型）
**自锁死**。所以多查询路径的正确形态是"**入口取一次读锁，把 `&TypeStore` 显式传下去**"：
既避免每查询一次取锁，也让"这个作用域只读"落在**签名**上——需要 intern 的代码拿不到
`&TypeStore` 就写不出那行调用。第二步（把锁换成快照式实现，如 `ArcSwap`）将来只需改
`TypeContext` 内部一处，因为调用面已经显式化了。

**② 本切片落地（display.rs）**：`FunctionDisplay`/`BlockDisplay`/`InstDisplay`/
`TerminatorDisplay`/`NameResolver`/`value_as_literal`/`fmt_phi_value` 的参数与字段由
`&TypeContext` 改为 `&TypeStore`；`Display for Module` 与 `function_to_string` 在入口
各取**一次**读锁（布局也直接读已取到的 store，避免 `Module::data_layout()` 再取一次）。
实测 `display.rs` 的非测试 `borrow()` 从 **51 处降到 2 处**（且这 2 处就是入口本身）。

**③ 度量与守卫**（新增 `tests/type_store_read_path.rs`，3 例）：`TypeContext` 增加
**仅 `debug_assertions`** 的读锁计数（`debug_read_count`/`debug_reset_read_count`，
release 下字段与方法都不存在 ⇒ 零开销），把纪律钉成可测不变量——打印一个模块、
一个函数各**只取 1 次**读锁，重复打印 3 次 = 3 次（不共享、不泄漏）。**负向验证**：
在 `Module::fmt` 里临时加一行 `self.types.borrow()` ⇒ 计数变 2、`module_display_takes_one_read_lock`
**FAILED**（报"实际 2 次"）；删掉后 3 例全绿。

**验证**：workspace 1470 passed / 0 failed / 19 ignored（+3）；x86 矩阵 195/3/0；
riscv64 131/67/0；fmt/clippy `-D warnings` 干净；`cargo doc -D warnings` 干净。
...
### S2（切片）：代码生成侧密集句柄表（`SecondaryMap`）+ `XReg` 不可密化的实测理由（2026-09-17）

S2 余项 ④ 的第一批：**键是密集句柄**的表换成 `SecondaryMap`（下标即句柄，不再逐次
SipHash）。
...
| 位置 | 表 | 键 |
| --- | --- | --- |
| `LowerCtx` | `vreg_classes` / `vreg_types` / `vreg_widths` | `VReg`（forge-ir 的 `EntityRef`） |
| `CompileState`（lowering） | `value_to_xreg` / `block_map` / `alloca_offsets` | `Value` / `Block` / `Inst` |

传参面同步：`TargetLowering::lower_terminator` 的两个映射参数改 `&SecondaryMap<…>`，
生成器模板（`forge-dsl` 的 `lower_terminator` 签名）改 `crate::prelude::SecondaryMap`，
`prelude` 增导出 `forge_ir::entity_map::SecondaryMap`；值语义键的取值点由 `.get(&x)`
改 `.get(x)`，并去掉模板里为 `HashMap::get` 而写的 `let cond_val = &cond_val;`。

**A/B 实测**（demo8 夹具 `compile_raw`，release，500 次 × 7 轮取中位数；临时探针跑完即删）：

- 64 条指令：**116.7 µs → 103.3 µs（−11.5%）**，样本区间不重叠（116.7±9 vs 103.3±6）；
- 256 条指令：412.7 µs → 400.5 µs（−3.0%），尾部与噪声重叠 ⇒ 只作方向性参考。

**先测后信**：首轮单次计时给出"256 条指令 +12%"的反向结果，重复测量（7 轮 × 500 次）
后判定为噪声——**单次计时不算证据**，这条也写进了守卫文件头。

**`XReg` 不可密化（如实记录，未做）**：`XReg { index, class }` 的 `Eq`/`Hash` **含 class**
——同一 index 配不同类是两个不同的键（`xreg_types`、`alloc_config::precolored`、
`AllocResult::{assignments, spill_slots}`、regalloc 的 `intervals`/`active` 都是 XReg 键）。
按 index 做下标的密集表会把它们**合并**，属语义变化 ⇒ 这些表保持 `HashMap`，直到先拍板
"class 是否属于键"。`entity_tables.rs::xreg_is_index_plus_class_so_not_dense` 把这条事实
钉成可执行断言（同一 index 不同类 ⇒ `HashMap` 里是两个条目）。

**守卫**（新增 `crates/backend/forge-codegen/tests/entity_tables.rs`，3 例）：
①`lowerctx_vreg_tables_are_dense`（编译期：三张表必须是 `SecondaryMap`，改成 `HashMap`
直接编不过）；②`compile_state_tables_are_dense`（源码级：三张 `pub(crate)` 字段必须是
`SecondaryMap`，`HashMap<` 回潮即红）；③`xreg_is_index_plus_class_so_not_dense`（XReg
键语义 + A/B 数据记录）。

**负向验证**：把 `value_to_xreg` 改回 `HashMap` ⇒ 编译失败（8 处调用点类型不符）；
